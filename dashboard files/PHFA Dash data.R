...
census_vars <- load_variables(year = 2024, dataset = "acs5")
...
dat_PA <- get_acs(geography = "state", 
                  variables = c("total_hh" = "B25011_001",
                                ...
                                "owner_inc_20k_35k" = "B25101_007" #20-35k 50%+
...
new_column_names <- paste0(names(dat24), 2024)

# Assign the modified column names back to the data frame
names(dat24) <- new_column_names

dat <- dat24  %>%
  rename(county = county2024) 
                                                                                                                    
#### CHAS Data   
tab8 <- read.csv("data/chas_pa_2018-2022/2018thru2022-050-csv/Table8.csv") %>%
  filter(st == 42) %>%
...
tab14b <- read.csv("data/chas_pa_2018-2022/2018thru2022-050-csv/Table14B.csv") %>%
  filter(st == 42) %>%
...
tab15c <- read.csv("data/chas_pa_2018-2022/2018thru2022-050-csv/Table15C.csv") %>%
  filter(st == 42) %>%
...
permits <- st_read("data/co2024a_clean.csv") %>%
  filter(FIPSState == 42) %>%
...
panel.sf <- dat %>%
  ...
  st_as_sf() %>%
  mutate(resunitpermitsrate24 = ifelse(housing_balance != 0, round(totalresunitpermits24/(total_housing_units2024/10000)), 0))

st_write(panel.sf, "dashboard files/phfa_dash_county_march_26.geojson", driver = "GeoJSON", delete_dsn = TRUE)

state_avg <- cbind(chas_pa, dat_PA, permits_pa) %>%
  mutate(resunitpermitsrate24 = ifelse(housing_balance != 0, round(totalresunitpermits24/(total_housing_units2024/10000)), 0))

st_write(state_avg, "dashboard files/phfa_dash_state_avg_march_26.csv", driver = "CSV", delete_dsn = TRUE)
